...
bool Viewport::touch_event(QTouchEvent *event)
{
  QList<QEventPoint> touchPoints = event->points();
	...
	if (!pinch_zoom_active_ ||
		...
		pinch_offset0_ = (double)(view_.offset() + view_.scale() * touchPoint0.position().x());
		pinch_offset1_ = (double)(view_.offset() + view_.scale() * touchPoint1.position().x());
...
void Viewport::wheelEvent(QWheelEvent *event)
{
	...
	if (event->angleDelta().y() == 0) {
		if (event->modifiers() & Qt::ControlModifier) {
			...
			view_.zoom(event->angleDelta().x() / 120.0, event->angleDelta().x());
		}
	} else if (event->angleDelta().x() == 0) {
		// Horizontal scrolling is interpreted as moving left/right
		view_.set_scale_offset(view_.scale(),
			event->angleDelta().y() * view_.scale() + view_.offset());
	}
}
